  ...
  files.forEach((f, i) => {
    ...
    li.innerHTML = `
      ...
      <div class="file-actions">
        <button class="icon-btn" data-action="up"   data-i="${i}" ${i === 0 ? 'disabled' : ''} aria-label="上へ">↑</button>
        <button class="icon-btn" data-action="down" data-i="${i}" ${i === files.length - 1 ? 'disabled' : ''} aria-label="下へ">↓</button>
        <button class="icon-btn icon-btn-danger" data-action="remove" data-i="${i}" aria-label="削除">×</button>
      </div>
    `;
...
async function mergePDFs() {
  ...
  try {
    ...
    for (let i = 0; i < files.length; i++) {
      ...
      setProgress(pct, `${i + 1} / ${files.length} 件目を処理中...`);
    ...
    processingSection.classList.add('hidden');
    fileListSection.classList.remove('hidden');
    alert('PDFの結合に失敗しました。\nパスワード保護されたPDFや破損したファイルは結合できません。');
  }
}
